...
import { audioDeviceConfigExists, loadAudioDeviceFromHomeConfig } from './select-audio';
...
export const execute = async (runConfig: Config): Promise<string> => {
    ...
    if (runConfig.audioReview?.file) {
        ...
        audioContext = await processAudioFile(runConfig.audioReview.file, runConfig);
    } else {
    ...
    try {
        ...
        if (process.platform === 'darwin') {
            ...
            try {
                ...
                const audioDevice = runConfig.audioReview?.audioDevice || homeDeviceConfig?.audioDevice || await detectBestAudioDevice();
